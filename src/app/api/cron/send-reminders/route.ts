...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || "https://www.task-trackers.com";
const FROM_EMAIL = "Task Tracker <noreply@task-trackers.com>";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Get all active reminder schedules with profile info
    ...
    for (const schedule of schedules) {
      ...
      const displayName =
        (schedule.profiles as { display_name: string | null })?.display_name ||
        "there";
      ...
      if (res.ok) {
        ...
        console.error(`Failed to send reminder to ${schedule.email_to}:`, err);
      }
    }
